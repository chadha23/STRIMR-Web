...
- Best Posting Hour: " . (count($stats['posts_by_hour']) > 0 ? array_search(max($stats['posts_by_hour']), $stats['posts_by_hour']) . ":00" : "N/A");
    ...
    private function generateSimpleInsights($stats) {
        ...
        if ($stats['total_posts'] > 0) {
            $insights[] = "📈 You've created {$stats['total_posts']} posts and received {$stats['total_likes']} total likes!";
            
            if ($stats['average_likes_per_post'] >= 10) {
                $insights[] = "🌟 Excellent engagement! Your posts average {$stats['average_likes_per_post']} likes each - you're creating highly engaging content!";
            } elseif ($stats['average_likes_per_post'] >= 5) {
                $insights[] = "✨ Great engagement! Your posts average {$stats['average_likes_per_post']} likes each. Keep up the good work!";
            } elseif ($stats['average_likes_per_post'] >= 2) {
                $insights[] = "👍 Good engagement with an average of {$stats['average_likes_per_post']} likes per post. Try experimenting with different content types!";
            } else {
                $insights[] = "💡 Your posts average {$stats['average_likes_per_post']} likes. Consider posting at different times or trying new content formats to boost engagement.";
            }
            
            if ($stats['most_liked_post'] && $stats['most_liked_post']['likes'] > 0) {
                $insights[] = "🏆 Your most popular post received {$stats['most_liked_post']['likes']} likes! Analyze what made it successful and create similar content.";
            }
            ...
            if (count($stats['posts_by_hour']) > 0) {
                ...
                $insights[] = "⏰ Your most active posting time is around {$bestHour}:00 ({$bestHourCount} posts). Your audience seems most engaged during this time!";
            }
            
            if ($stats['engagement_rate'] > 50) {
                $insights[] = "🎯 Your engagement rate is {$stats['engagement_rate']}% - this is outstanding! Your content resonates well with your audience.";
            } elseif ($stats['engagement_rate'] > 20) {
                $insights[] = "📊 Your engagement rate is {$stats['engagement_rate']}% - good performance! There's room to grow even more.";
            } else {
                $insights[] = "📊 Your engagement rate is {$stats['engagement_rate']}%. Try posting more consistently and engaging with your audience to improve this metric.";
            }
        } else {
            $insights[] = "🚀 Start posting to see your statistics and AI-powered insights!";
        }
